...
import { EventsViewToggle } from "@/components/public/events-view-toggle";
...
export default async function EventsPage({
  ...
  const calendarEvents = events.map((event) => {
    const slug =
      locale === "en" && event.slugEn ? event.slugEn : event.slugJa;
    const title =
    ...
    <div className="mx-auto max-w-4xl px-4 py-16 sm:px-6 lg:px-8">
      ...
      <div className="mt-6 flex flex-wrap items-center justify-between gap-4">
        ...
        <EventsViewToggle
          locale={locale}
          labels={{
            list: dict.events?.viewList ?? "List",
            calendar: dict.events?.viewCalendar ?? "Calendar",
          }}
        />
      </div>
